...
def main() -> None:
    ...
    # AMASS files commonly contain object-valued metadata (for example
    # ``gender``).  Loading every member would therefore require pickle even
    # though playback only needs these two numeric arrays.  Keep pickle
    # disabled and read only the fields used below.
    with np.load(args.motion_file, allow_pickle=False) as motion:
        missing = {"poses", "trans"}.difference(motion.files)
        if missing:
            raise ValueError(
                f"Motion file {args.motion_file!r} is missing required fields: "
                f"{sorted(missing)}"
            )
    ...
    if poses.ndim != 2 or poses.shape[1] < 72:
        raise ValueError(f"Expected poses with shape [frames, >=72], got {poses.shape}")
    if trans.ndim != 2 or trans.shape[1] != 3 or len(trans) != len(poses):
        raise ValueError(
            f"Expected trans with shape [{len(poses)}, 3], got {trans.shape}"
        )
